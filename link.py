...
class Link:
    def __init__(self,node_x,node_y,network_event_scheduler,bandwidth=10000,delay=0.001,packet_loss = 0.0):
        self.node_x = node_x
        self.node_y = node_y
    ...
    def transfer_packet(self,from_node):
        ...
        if queue:
            ...
            if queue:
                next_packet = queue[0][0]
                self.network_event_scheduler.schedule_event(next_packet, self.transfer_packet, from_node)
    ...
    def enqueue_packet(self,packet,from_node):
        ...
        packet_transfer_time = packet.size * 8 / self.bandwidth
        deque_time = self.network_event_scheduler.current_time + current_queue_time

        heapq.heappush(queue,(deque_time,packet,from_node))
        self.add_to_queue_time(from_node,packet_transfer_time)
        if len(queue) == 1:
            self.network_event_scheduler.schedule_event(deque_time,self.transfer_packet,from_node)
