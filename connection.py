    ...
    def send_packet(self,packet):
        if packet.destination == self.address:
            ...
            for link in self.links:
                ...
                print(f"ノード{self.node_id} -> ノード{next_node.node_id}:パケット送信")
...
node1 = Node(1,"192.168.1.1",network_graph)
node2 = Node(2,"192.168.1.2",network_graph)
node3 = Node(3,"192.168.1.3",network_graph)
node4 = Node(4,"192.168.1.4",network_graph)
node5 = Node(5,"192.168.1.5",network_graph)
node6 = Node(6,"192.168.1.6",network_graph)

link_1to2 = Link(node1,node2,network_graph)
link_2to3 = Link(node2,node3,network_graph)
link_3to4 = Link(node3,node4,network_graph)
link_3to5 = Link(node3,node5,network_graph)
link_4to6 = Link(node4,node6,network_graph)

packet = Packet(node1.address,node6.address,"Hello Stranger!!")
