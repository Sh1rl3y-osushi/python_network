...
class NetworkEventScheduler:
    ...
    def print_packet_logs(self):
        for packet_id, log in self.packet_logs.items():
            print(f"Packet ID: {packet_id} Src: {log['source']}{log['create_time']} --> Dst: {log['destination']}{log['arrival_time']} Size: {log['size']}")
            for event in log['events']:
                print(f"Time : {event['time']} Event: {event['event']} Node: {event['node_id']}")
    ...
    def generate_summary(self,packet_logs):
        ...
        for packet_i,log in packet_logs.items():
            ...
            summary_data[src_dst_pair]["min_creation_time"] = min(summary_data[src_dst_pair]["min_creation_time"],log["create_time"])
            ...
            if "arrival_time" in log and log["arrival_time"] is not None:
                ...
                summary_data[src_dst_pair]["total_delay"] += log["arrival_time"] - log["create_time"]
        ...
        for src_dst,data in summary_data.items():
            sent_pacekts = data["sent_packets"]
            ...
            print(f"Source: {src_dst[0]}, Destination: {src_dst[1]}")
            print(f"Tolal Sent Packets: {sent_pacekts}")
    ...
    def generate_throughput_graph(self,packet_logs):
        ...
        min_time = min(log['create_time'] for log in packet_logs.values())
